...
//     announced-to), a cheap force graph polled live, no LLM.
//   • the considered VOICE (1b): a slow reflector that reads the skeleton every ~25 min and lays
//     meaning over it — the deduced goal, the threads in flight, tensions worth flagging, and the
//     semantic edges (feeds / duplicates / loops-back) that only meaning can see.
// It is a mirror: it shows me to myself. It never steers a part.
...
async function load() {
  try {
    const since = Date.now() - windowMs.value
    const [s, a] = await Promise.all([
      api.selfSchema({ since }),
      api.selfAssessment().catch(() => null)
    ])
    schema.value = s
    assessment.value = a
  } catch (_) {} finally { loading.value = false }
}
function setWindow(k) { windowKey.value = k; loading.value = true; load() }
...
// Body at a glance — surface breakdown + live repos + total load, all from the current node set.
...
// Semantic edges from the reflector, kept only where BOTH endpoints are in the current graph.
const semanticEdges = computed(() => {
  if (!latest.value?.relations) return []
  const present = new Set(nodes.value.map((n) => n.session_id))
  return latest.value.relations.filter((r) => present.has(r.from) && present.has(r.to))
})
...
const option = computed(() => {
  ...
  const surfaceList = [...new Set(s.nodes.map((n) => n.surface))]
  ...
  const structural = s.edges.map((e) => ({
  ...
  const semantic = semanticEdges.value.map((e) => ({
  ...
  return {
    tooltip: {
      ...
      formatter: (p) => p.dataType === 'edge'
        ...
        : `<b>${surfaceMeta(p.data.surface).label}</b><br/>${p.data._activity || '(idle)'}<br/><span style="color:#94a3b8">${p.data._repo || ''} ${p.data._age}</span>`
    },
    ...
    series: [{
      ...
      data: s.nodes.map((n) => ({
        ...
        // resting limbs (idle a while) fade; the working ones stay bright
  ...
  <div>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
      ...
      <div class="glass p-2 lg:col-span-2">
        <p v-if="loading && !schema" class="py-24 text-center text-sm text-slate-500">sensing…</p>
        ...
        <p v-else-if="nodes.length === 1" class="py-24 text-center text-sm text-slate-400">
          One part active: <span class="text-slate-200">{{ nodeLabel(nodes[0]) }}</span>. A single limb — nothing to relate yet.
        </p>
        <BaseChart v-else :option="option" height="560px" />
      </div>

      <!-- the parts, listed — so even a quiet body reads clearly -->
      <div class="glass flex flex-col p-3">
        <div class="mb-2 flex items-center justify-between">
          ...
          <span class="text-[11px] text-slate-600">sized by load</span>
        </div>
        <div class="flex-1 space-y-1.5 overflow-y-auto" style="max-height: 560px">
          ...
          <div v-for="n in nodes" :key="n.session_id" class="rounded-lg border border-white/5 bg-white/[0.02] p-2"
               :class="n.age_min > 30 ? 'opacity-60' : ''">
